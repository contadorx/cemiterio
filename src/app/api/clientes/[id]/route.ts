...
export async function GET(_req: NextRequest, { params }: { params: { id: string } }) {
  ...
  const [{ data: tumulos }, { data: planos }, { data: mov }, { data: msgs }] = await Promise.all([
    db.from("tumulos").select("id,identificacao,numero,falecido_nome,datas_gatilho,qr_token,rua,rua_id,codigo,ordem_na_rua,valor_lavagem,valor_base,inicio_cobranca,periodicidade,freq_pagamento,contratado,quadra_id,lat,lng,gps_precisao,gps_amostras,foto_referencia_url,foto_enquadramento_url,familia_id,ruas(nome),quadras(codigo)").eq("cliente_id", id),
