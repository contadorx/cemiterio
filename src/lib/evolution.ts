...
// Envia uma imagem (por URL pública ou base64) com legenda.
export async function enviarWhatsappMidia(
  ...
  const numero = normalizarTelefone(telefone);
  const url = `${env.evolutionUrl()}/message/sendMedia/${env.evolutionInstance()}`;
  ...
  if (!res.ok) {
    const detalhe = await res.text().catch(() => "");
    throw new Error(`Evolution sendMedia falhou (${res.status}): ${detalhe}`);
  }
}
